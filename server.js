...
let favicon
let newsFilter
let colorOptionsArray = ["after solar noon but before sunset", "after sunrise but before solar noon", "before sunrise", "before nadir"]
let timeStatus
       if (dateNow > sunrise && dateNow < sunset && dateNow > noon){
          colorOptionsArray.splice(0, 1)
          timeStatus = "it's after solar noon but before sunset"
          accentColor = "black"
          ...
          colorOptionsArray.splice(1, 1)
          timeStatus = "it's after sunrise but before solar noon"
          accentColor = "black"
          ...
          colorOptionsArray.splice(2, 1)
          timeStatus = "it's before sunrise"
          ...
          colorOptionsArray.splice(3, 1)
          timeStatus = "it's after sunset and before nadir"
          backgroundColor = "black"
   ...
   return {backgroundColor, accentColor, favicon}     
  }
...
console.log(`https://geolocation-db.com/json/ee28a1b0-e604-11eb-9948-af584490f811/${ip}`)
var data = request(`https://geolocation-db.com/json/ee28a1b0-e604-11eb-9948-af584490f811/${ip}`, function (error, response, body) {
  let data = JSON.parse(body.replace(/&quot;/g,'"'))
  let favicon
  console.log(data)
  const colors = getTimes(data)
  favicon = colors.favicon
  data.backgroundColor = colors.backgroundColor
  data.accentColor = colors.accentColor
  console.log(data)
